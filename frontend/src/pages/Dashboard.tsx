...
    
    // 1. Determinar o termo de busca da API
...
    // 3. NÃO BUSCAR se estiver no Nível 2 (Agregação Local)
...
      const dateParams = { startDate: (start || startDate)?.toISOString(), endDate: (end || endDate)?.toISOString() };
      ...
      // <<< MUDANÇA PRINCIPAL (CORREÇÃO DA PAGINAÇÃO)
      // Parâmetros básicos de data e busca
      const dataParams: any = {
        ...dateParams,
        search: apiSearchTerm || undefined
      };

      // Adiciona paginação SOMENTE se estiver no Nível 3 (selectedEditor) 
      // ou na Busca Manual (search)
      if (selectedEditor || search) {
        dataParams.page = page;
        dataParams.limit = limit;
      }
      // Se for Nível 1 (sem editor ou busca), 'page' e 'limit' não são enviados.
      // <<< FIM DA MUDANÇA

      // 1. Chamada de dados principal (/data)
      const dataRes = await axios.get(`${API_URL}/data`, { params: dataParams });
      setData(dataRes.data.data); // Sempre atualiza 'data'
      setTotalCount(dataRes.data.total); 
      
      // 2. Salvar no cache APENAS se for uma busca de Nível 1
      if (!apiSearchTerm && !selectedCargo) {
        setNivel1DataCache(dataRes.data.data);
      }
      
      // 3. Chamadas de Stats/Ranking/Eixos (SÓ CARREGA SE NÃO HOUVER FILTRO ATIVO)
      if (!apiSearchTerm && !selectedCargo) { 
...
  
  // ==========================================================
  // LÓGICA DE RECARGA: (sem mudanças)
  // ==========================================================
  useEffect(() => { 
    fetchData(); 
  }, [fetchData]); 

  // === FUNÇÕES DE NAVEGAÇÃO E BUSCA (sem mudanças) ===
  
  const handleSearch = () => { 
...
            
            {/* Seção de Filtros/Datas (sem mudanças) */}
            <Box mb={4}>
...
            {/* ESTATÍSTICAS + GRÁFICOS (sem mudanças) */}
            {!loading && !selectedCargo && !selectedEditor && !search && ( 
              <>
               {/* Cards de Estatísticas (Seu código de cards mantido) */}
...
              fetchData={fetchData} // mantido para o caso da paginação no Nível 3
              // PROPS DE NAVEGAÇÃO
